...
export function NovaFvs({ onSuccess }: NovaFvsProps) {
  ...
  const servicos = ['Estrutura', 'Alvenaria', 'Revestimento Cerâmico', 'Pintura', 'Impermeabilização']
  const pavimentos = ['Fundação', 'Subsolo', '1º Pavimento', '2º Pavimento', '3º Pavimento', 'Cobertura']
      ...
      <HalstenCard>
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-6)' }}>
          ...
          <div style={{ paddingBottom: 'var(--sp-6)', borderBottom: `1px solid var(--bg-2)` }}>
            <h2 style={{ fontSize: 14, fontWeight: 700, color: 'var(--ink)', marginBottom: 'var(--sp-4)', margin: 0 }}>
              📋 Identificação
            </h2>
          ...
          <div style={{ paddingBottom: 'var(--sp-6)', borderBottom: `1px solid var(--bg-2)` }}>
            ...
            <HalstenSelect
              ...
              onChange={handleChange}
              options={EMPREENDIMENTOS.map(e => e.nome)}
              required
            />
