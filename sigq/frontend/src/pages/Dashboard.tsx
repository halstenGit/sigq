  ...
  return (
    <div style={{ maxWidth: 1400, margin: '0 auto', padding: 'var(--sp-8)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 'var(--sp-4)', marginBottom: 'var(--sp-8)' }}>
        {kpis.map(k => (
          <div key={k.label} style={{ padding: 'var(--sp-6)', background: 'var(--bg-1)', border: '1px solid var(--bg-2)', borderRadius: 4 }}>
        ...
        <HalstenCard>
            ...
            {chartData.map(d => (
              <div key={d.mes} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'var(--sp-2)' }}>
                ...
                <div style={{ width: '100%', display: 'flex', gap: 2, alignItems: 'flex-end', height: 90 }}>
                  <div style={{ flex: 1, background: d.fvs ? 'var(--ink)' : 'var(--bg-2)', height: `${(d.fvs / 38) * 90}px`, borderRadius: 2 }} />
                  <div style={{ flex: 1, background: d.nc ? 'var(--ink-2)' : 'var(--bg-2)', height: `${(d.nc / 6) * 90}px`, borderRadius: 2, opacity: 0.6 }} />
                </div>
          ...
          <div style={{ display: 'flex', gap: 'var(--sp-4)', marginTop: 'var(--sp-4)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-2)', fontSize: 11, color: 'var(--ink-2)' }}>
              <div style={{ width: 8, height: 8, borderRadius: 1, background: 'var(--ink)' }} />
              FVS realizadas
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-2)', fontSize: 11, color: 'var(--ink-2)' }}>
              <div style={{ width: 8, height: 8, borderRadius: 1, background: 'var(--ink-2)', opacity: 0.6 }} />
              Com NCs
            </div>
          </div>
        </HalstenCard>
